(function () {
	...
	$("#hpbartext").SetPanelEvent("onactivate", function(){
		if (GameUI.IsAltDown()){
			...
			if (localplayer == unit) {Game.ServerCmd(`say_team ${$.Localize("#DOTAold_Ping_Health_self",$("#hpbartext"))}`)};
			if (Entities.IsEnemy(unit)) {Game.ServerCmd(`say_team ${$.Localize("#DOTAold_Ping_Health_enemy",$("#hpbartext"))}`)};
		};
	});
