...
var AbilityPanel = (function () {
	function AbilityPanel(parent, ability, unit) {
		...
		if ((Abilities.IsPassive(this.ability) || Abilities.GetLevel(this.ability) < 1) && !this.learning) {
			this.panel.FindChildTraverse("HotkeyLabel").style.visibility = "collapse";
		} else {
			this.panel.FindChildTraverse("HotkeyLabel").style.visibility = "visible";
	...
	AbilityPanel.prototype.update = function () {
		...
		if ((Abilities.IsPassive(this.ability) || Abilities.GetLevel(this.ability) < 1) && !this.learning) {
			...
			this.panel.FindChildTraverse("HotkeyLabel").style.visibility = "visible";
			this.panel.FindChildTraverse("HotkeyLabel").text = Abilities.GetKeybind(this.ability);
		}
		if (state !== this.state) {
